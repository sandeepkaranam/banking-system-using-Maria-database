...
@Contact_no.,@Work_no.)
SET first_name=@First_Name, last_name=@last_name,DOB=STR_TO_DATE(@DOB, '%m/%d/%y'),age=@Age,sex=@Sex,street=@Street,
city=@City,province=@Province,zipcode=@Zipcode,created_time=STR_TO_DATE(@Created_Time, '%m/%d/%Y %H:%i')
,contact_no=@Contact_no.,work_no=@Work_no.;
...
INSERT INTO transaction (transaction_id, transaction_amount, date_of_transaction, total_balance, tran_type_id, source_account_id, destination_account_id)
